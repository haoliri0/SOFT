...
static __device__
void op_compute_decomp_pivot(const ArgsComputeMeasureCondition args, const DimsIdx<1> dims_idx) {
...
void cuda_compute_decomp_pivot(
    ...
    cuda_dims_op<ArgsComputeMeasureCondition, 1, op_compute_decomp_pivot>
...
void Simulator::measure(const Qid target, Bit *res, Flt *prob) const noexcept {
    ...
    cuda_compute_decomp_pivot(stream, shots_state_ptr, target);
